...
PEN toolboxの三つの反応（活性化、自己触媒反応、阻害）に加えて捕食反応(Predator Prey=PP)も使用できる化学反応ネットワーク設計・シミュレーションシステムを実装した。・PEN toolboxは、任意の化学反応ネットワークを作るためのフレームワークである。化学反応ネットワークは分子群ロボットのコントローラの設計に使われる。
