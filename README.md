...
PEN toolboxの三つの反応（活性化、自己触媒反応、阻害）に加えて捕食反応(Predator Prey=PP)も使用できる化学反応ネットワーク設計・シミュレーションシステムを実装した。(使用言語：Python)
...
# 使い方
(※(c)の入力方法が原始的で使いづらいため現在改良中)

同じレポジトリ内のPEN_PP.ipynbを実行する。(Jupyter notebookで実行）
...
③PEN＿PP.ipnbを開くと実行結果が確認できます。

ここまでで既存コードの実行結果の確認ができる。
以下に物質の濃度など条件を変えたいときや、新しいネットワークを入力したい場合の方法を記す。
